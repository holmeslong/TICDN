...
class SEBlock(nn.Module):
    def __init__(self, channel, ratio=8) -> None:
        super().__init__()
...
class SESpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        super(SESpatialAttention, self).__init__()
    ...
    def forward(self, x):
        ...
        #std_out, _ = np.std(x, dim=1, keepdim=True)
        std_out=(max_out-avg_out)*0.5
...
class DRN_A(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        # 针对原图做如下处理：
        # 1. 降低分辨率
        # 2. 将原图的rgb通道与layer1的结果的256个特征通道进行通道叠加
        # 3. 添加通道注意力
        # 4. 将259个通道通过1x1的卷积操作改为256个通道，用于后续处理
        self.avg_pool = nn.AvgPool2d(2, 2)
        self.channel_attn = SEBlock(256 + 3)
        self.change_seq = nn.Sequential(
            nn.Conv2d(259, 256, 1, 1, bias=False),
            BatchNorm(256),
            nn.ReLU(inplace=True),
        )

        # self.channel_attn = SEBlock(256)
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = torch.cat([x, rgb_img], dim=1)  # 是split和chunk的逆操作
    ...
    def _freeze_stages(self):
        ...
        m = getattr(self, 'layer1')  # 冻结第一层
        for param in m.parameters():
            param.requires_grad = False

    def train(self, mode):
        super(DRN_A, self).train(mode)
        self._freeze_stages()
        if mode:
            for m in self.modules():
